	...
	trans_calib(1, 2, 0)
{
    ...
    connect(ui.camdevice_combo, &QComboBox::currentTextChanged, this, &TrackerDialog_WII_PT::set_camera_settings_available);
    set_camera_settings_available(ui.camdevice_combo->currentText());
    connect(ui.camera_settings, &QPushButton::clicked, this, &TrackerDialog_WII_PT::show_camera_settings);

    connect(&timer, &QTimer::timeout, this, &TrackerDialog_WII_PT::poll_tracker_info_impl);
    ...
    connect(this, &TrackerDialog_WII_PT::poll_tracker_info, this, &TrackerDialog_WII_PT::poll_tracker_info_impl, Qt::QueuedConnection);
}
...
void TrackerDialog_WII_PT::set_camera_settings_available(const QString& camera_name)
{
    const bool avail = video_property_page::should_show_dialog(camera_name);
    ui.camera_settings->setEnabled(avail);
}

void TrackerDialog_WII_PT::show_camera_settings()
{
...
void TrackerDialog_WII_PT::save()
{
    s.b->save();
}

void TrackerDialog_WII_PT::doOK()
{
    save();
    close();
}

void TrackerDialog_WII_PT::doCancel()
{
    close();
}

void TrackerDialog_WII_PT::register_tracker(ITracker *t)
{
    tracker = static_cast<Tracker_WII_PT*>(t);
    ui.tcalib_button->setEnabled(true);
    poll_tracker_info();
    timer.start();
}

void TrackerDialog_WII_PT::unregister_tracker()
{
    tracker = NULL;
